...
inline void Kalman_Init(Kalman *kalPointer)
{
  ...
  kalPointer->P[0][0] = 0;                           // Since we assume that the bias is 0 and we know the starting angle (use setAngle), 
  kalPointer->P[0][1] = 0;                           // the error covariance matrix is set like so - see: http://en.wikipedia.org/wiki/Kalman_filter#Example_application.2C_technical  
  kalPointer->P[1][0] = 0;
  kalPointer->P[1][1] = 0;
}
